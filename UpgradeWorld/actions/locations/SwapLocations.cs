...
public class SwapLocations : EntityOperation {
  ...
  private void Swap(IEnumerable<string> ids, DataParameters args) {
    ...
    var toModify = locs.Where(kvp => prefabs.Contains(kvp.Value.m_location?.m_hash ?? 0) || prefabs.Contains(kvp.Value.m_location?.m_prefabName.GetStableHashCode() ?? 0)).ToArray();
